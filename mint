...
SSH_PORT=22  # SSH port set to 22 as per default
...
echo "[*] Adding 'mariya' to 'pioneers' group..."
if id "mariya" &>/dev/null; then
    usermod -aG pioneers "mariya"
    if [ $? -eq 0 ]; then
        echo "[+] 'mariya' added to 'pioneers' group successfully."
    else
...
# 5. Ensure UFW is the only firewall installed
echo "[*] Ensuring UFW is the only firewall installed..."
...
if dpkg -l | grep -qw firewalld; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "[-] Failed to remove firewalld."
    fi
else
...
# 16. Remove Unnecessary Packages
...
# 18. Install Required Packages (Firefox, Thunderbird, Perl, X2GO Server)
echo "[*] Installing required packages..."

# Remove SNAP version of Firefox
echo "[*] Removing SNAP version of Firefox if installed..."
snap remove firefox --purge &>/dev/null
if [ $? -eq 0 ]; then
    echo "[+] SNAP version of Firefox removed."
else
    echo "[+] SNAP version of Firefox not installed or already removed."
fi
...
if [ $? -eq 0 ]; then
    echo "[+] Mozilla PPA added successfully."
else
    echo "[-] Failed to add Mozilla PPA."
fi

# Set preference to prevent installation of SNAP version
echo "[*] Configuring APT preferences to use Mozilla PPA for Firefox..."
...
if [ $? -eq 0 ]; then
    ...
    echo "[-] Failed to configure APT preferences."
fi

# Install Firefox from the PPA
echo "[*] Installing latest stable version of Firefox from Mozilla PPA..."
apt update
apt install -y firefox
if [ $? -eq 0 ]; then
    echo "[+] Firefox installed successfully from Mozilla PPA."
else
    echo "[-] Failed to install Firefox."
fi

# Install Thunderbird and Perl
echo "[*] Installing Thunderbird and Perl..."
apt install -y thunderbird perl
if [ $? -eq 0 ]; then
    echo "[+] Thunderbird and Perl installed successfully."
else
    echo "[-] Failed to install Thunderbird and Perl."
fi
...
# 22. Ensure sshd service is enabled
